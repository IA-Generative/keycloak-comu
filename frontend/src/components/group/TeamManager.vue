...
import type { ErrorMessageKey } from '@/shared/ErrorMessages'
import { TeamNameSchema } from '@/shared/schemas'
import type { GroupDtoType, UserDtoType } from '@/shared/types'
...
const rootGroupPrefix = computed(() => appConfig.value.rootGroupPath)

function sortFunction<T extends Record<string, any>>(array: Array<T>, key: keyof T, fallbackKey: keyof T): T[] {
...
const sortedTeams = computed(() => {
  return sortFunction(group.value.teams ?? [], 'name', 'id').map(team => ({
    ...
    members: sortFunction(
      (team.members ?? []).map(m => groupMembersSorted.value.find(u => u.id === (typeof m === 'string' ? m : m.id))).filter(Boolean) as UserDtoType[],
      'first_name', 'email',
    ),
  }))
})
...
// list of user ids currently highlighted
...
const actual = computed(() => {
  return (group.value.teams ?? []).reduce((acc, sg) => {
    acc[sg.name] = Array.from(new Set(sg.members?.map((m: any) => typeof m === 'string' ? m : m.id) ?? []))
    return acc
...
async function createTeam() {
  ...
  await backend.editTeam(group.value.id, newGroupName.value, [])
    .then(() => {
      newGroupName.value = ''
...
async function deleteTeam(name: string, confirm = false) {
  ...
  try {
    ...
    await backend.deleteTeam(group.value.id, name)
  } finally {
...
watch(wanted, async (newVal) => {
  ...
  for (const [name, users] of wantedEntries) {
    ...
    try {
      await backend.editTeam(group.value.id, name, Array.from(userSet.values()))
    } catch (e) {
...
<template>
  <div class="fr-container">
    <!-- Élément draggable -->
    <div v-if="Object.keys(group.teams ?? {}).length" class="flex flex-row flex-wrap gap-4 mb-4">
      <template v-if="canManage">
        <DsfrTag v-for="member in groupMembersSorted" :key="member.id"
          :label="`${member.first_name} ${member.last_name}`" draggable="true" selectable
          :aria-pressed="selectMode && selectedTags.includes(member.id) ? 'true' : 'false'"
          @dragstart="onDragStart($event, member.id)" @click="selectTag(member.id)" />
      </template>
    </div>

    <!-- Zone de dépôt stylée DSFR -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      <div v-if="canManage" class="fr-mb-2w new-group group">
        <form class="flex flex-row gap-2" @submit.prevent="createTeam">
          <div>
            <DsfrInput v-model="newGroupName" placeholder="Nom de l'équipe" label="Créer une nouvelle équipe"
              label-visible />
          </div>
          <DsfrButton class="self-start fr-mt-3w" title="Créer le groupe" :disabled="!newGroupNameValidation.success">
            +
          </DsfrButton>
        </form>
        <span v-if="!newGroupNameValidation.success && newGroupName" class="fr-text--xs fr-error-text">{{
          ERROR_MESSAGES[newGroupNameValidation.error.issues[0]?.message as ErrorMessageKey]?.fr }}</span>
      </div>
      <div v-for="(team) in sortedTeams" :key="team.id" class="fr-mb-2w group flex flex-col relative gap-4"
        :class="{ 'fr-upload--dragover': currentDragged === team.name }" @dragover.prevent="onDragOver(team.name)"
        @dragleave="onDragLeave(team.name)" @drop="onDrop($event, team.name)">
        <div v-if="teamStatus[team.name] === 'pendingDelete'"
          ...
          <div class="flex flex-row gap-2">
            <DsfrButton secondary title="Annuler la suppression" @click="() => { teamStatus[team.name] = 'idle' }">
              Annuler
            </DsfrButton>
            <DsfrButton class="bg-red-600 border-red-600 hover:bg-red-700 hover:border-red-700"
              title="Confirmer la suppression" @click="() => deleteTeam(team.name, true)">
              Confirmer
            </DsfrButton>
          </div>
        </div>
        <div class="w-full flex flex-col h-full gap-4"
          ...
          <div class="flex flex-wrap justify-between align-start w-full">
            <div class="fr-mb-0 text-wrap fr-text--lg font-bold">
              {{ team.name }}
            </div>
            <DsfrTag v-if="canManage" icon-only selectable icon="ri-close-line"
              @click.capture="() => deleteTeam(team.name, false)" />
          </div>
          <div class="grow">
            <label v-if="!team.members.length" class="fr-label text-sm italic">{{ emptyGroupMessage }}</label>
            <div v-else class="fr-upload__msg">
              <div class="flex flex-row  flex-wrap gap-4">
                <DsfrTag v-for="user in team.members.toSorted()" :key="user.id"
                  :icon="canManage ? 'ri-close-line' : undefined" :selectable="canManage"
                  :label="`${user.first_name} ${user.last_name}`" :class="usersClass[user.id]?.[team.name]"
                  @click="() => removeUserFromTeam(user.id, team.name)" />
              </div>
            </div>
          </div>
          <div class="flex flex-row justify-between w-full group-footer">
            <label class="fr-label text-sm italic align-end opacity-75">
              {{ rootGroupPrefix }}{{ group.name }}/{{ team.name }}
            </label>

            <div v-if="selectMode && selectedTags.length">
              <DsfrButton class="" secondary size="sm" label="Ajouter ici" @click="() => addSelectedTags(team.name)" />
            </div>
          </div>
        </div>
      </div>
    </div>
    <div v-if="!canManage && !(group.teams ?? []).length" class="text-center w-full mt-5">
      <p>
