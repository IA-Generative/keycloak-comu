...
export function generateGroupExpirationEmailDSFR(group: GroupDetails, expiresAt: string): string {
  const groupLink = `${baseUrl}/g/${group.id}`
  const formattedDate = new Date(Number(expiresAt)).toLocaleDateString()
  return `
  <!DOCTYPE html>
  <html lang="fr">
    <head>
      <meta charset="UTF-8" />
      <title>Le groupe ${group.name} arrive à expiration</title>
    </head>
    <body style="font-family: Arial, 'Marianne', sans-serif; color: #161616; background-color: #f6f6f6; padding: 24px;">
      <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="max-width: 600px; margin: auto; background: #ffffff; border-radius: 8px; overflow: hidden; border: 1px solid #ddd;">
        <tr>
          <td style="padding: 20px; text-align: center; background-color: #ff9940; color: #000;">
            <h1 style="margin: 0; font-size: 22px; font-weight: 700;">Expiration prochaine du groupe</h1>
          </td>
        </tr>
        <tr>
          <td style="padding: 24px;">
            <p style="font-size: 16px; line-height: 1.5;">Bonjour,</p>
            <p style="font-size: 16px; line-height: 1.5;">
              Le groupe <strong style="color: #000091;">${group.name}</strong> arrivera à expiration le 
              <strong>${formattedDate}</strong>.
            </p>
            <p style="font-size: 16px; line-height: 1.5;">
              Pour conserver ce groupe et éviter sa suppression, merci de le renouveler avant cette date.
            </p>
            <p style="text-align: center; margin: 32px 0;">
              <a href="${groupLink}" 
                 style="display: inline-block; background-color: #000091; color: #fff; text-decoration: none; padding: 14px 24px; border-radius: 4px; font-weight: bold; font-size: 16px;">
            ...
            <p style="font-size: 16px; line-height: 1.5;">
              Merci de votre vigilance,<br/>L’équipe
            </p>
          </td>
        </tr>
      </table>
