...
<template>
  <div>
    <div>
      <div v-for="(link, idx) in links" :key="idx" class="flex flex-row gap-2 items-start">
        <DsfrInputGroup v-if="editingLinks" :id="idx === links.length - 1 ? 'last-input' : ''" v-model="links[idx]"
          class="grow" :error-message="extractErrorMessage(links[idx])" label="Lien associé au groupe"
          placeholder="Ajouter un lien pertinent pour les membres du groupe (site web, documentation, etc.)." type="url"
          @keyup.enter="addLink" />
        <a v-else :href="link" class="mb-2" target="_blank" rel="noopener noreferrer">{{ link }}</a>
        <DsfrButton v-if="editingLinks" tertiary icon-only icon="ri-close-line" @click="removeLink(idx)" />
      </div>
      <div v-if="!links.length && !editingLinks">
        Aucun lien associé à ce groupe pour le moment.
      </div>
    </div>
    <div class="mb-4">
      <DsfrButton v-if="editingLinks" :disabled="links.slice(-1)[0] === ''" tertiary icon="ri-add-line"
        @click="addLink">
        Ajouter un lien
      </DsfrButton>
    </div>
    <template v-if="canEdit && editingLinks">
      <DsfrButton tertiary icon="ri-edit-line" label="Annuler" @click="cancelEditLinks" />
      <DsfrButton class="fr-ml-2w" icon="ri-edit-line" label="Sauvegarder" @click="saveLinks" />
    </template>
    <template v-else-if="canEdit">
      <DsfrButton tertiary class="fr-mt-2w" icon="ri-edit-line" label="Modifier les liens" @click="startEditingLinks" />
    </template>
  </div>
</template>
