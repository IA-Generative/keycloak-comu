...
<template>
  <div class="fr-container fr-mt-4w">
    ...
    <div class="flex justify-between items-center mb-4 mt-8">
      <h2>Vos Groupes</h2>
      <div>
        <DsfrButton tertiary @click="dashboardStore.getGroups">
          Actualiser
        </DsfrButton>
      </div>
    </div>
    <div class="gap-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
      ...
      <DsfrTile v-for="group in groups.joined" :key="group.id" :to="`/g/${group.id}`" :title="group.name" horizontal />
      <DsfrTile v-for="group in groups.requested" :key="group.id" :to="`/g/${group.id}`" :title="group.name"
        description="Demande en attente" horizontal />
    </div>
  </div>
</template>
