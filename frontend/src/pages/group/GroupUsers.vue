...
<template>
  <div>
    ...
    <div v-if="[0, 20, 30].includes(mylevel)" class="flex lg:flex-row flex-col gap-16">
      <!-- Formulaire d'ajout de membre -->
      <div v-if="mylevel >= 20" class="grow">
        <InviteMember :group-id="group.id" />
      </div>
      <div class="grow">
        <InvitesList v-if="mylevel >= 20" />
      </div>
    </div>
  </div>
</template>
